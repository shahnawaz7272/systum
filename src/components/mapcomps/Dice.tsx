...
const Dice = ({ color }: color) => {
  ...
  const IsAllTokensinHome = TokensOfThisColor.filter(
    (t) => t.position === "home" || t.isFinished === true
  );
  ...
  return (
    <div
      className={`bg-white h-[75px] w-[75px] rounded-[8px] shadow-md flex justify-center items-center flex-wrap cursor-pointer transition-transform duration-300 ${
      ...
      {dicerollarr.map((_, index) => {
        if (NumberOnDice === 5) {
          const fiveDotStyles = [
            ...
            "justify-center items-center w-[46px]",
